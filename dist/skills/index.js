...
import { GLOBAL_DIR, loadProjectSkills } from '../utils/settings';
...
// 从目录加载 skills
function loadSkillsFromDir(dir, skills) {
...
// 获取skill定义
export function getSkill(name, workspacePath) {
    return loadSkills(workspacePath).get(name) || null;
}
// 检查输入是否是skill调用
export function parseSkillInput(input, workspacePath) {
...
// 解析模板变量
function parseTemplateArgs(template, input) {
    if (!template) {
        return { input };
    }
...
// 列出所有skills
export function listSkills(workspacePath) {
...
// 获取包中的 skills 列表
function getPackageSkills(pkgName) {
...
// 卸载 skill 包
export async function uninstallSkill(packageName) {
    try {
...
// 保存单个 skill（写入到指定包目录）
export async function saveSkill(skillName, skill, pkgName = 'custom') {
    try {
        const pkgDir = join(SKILLS_DIR, pkgName);
...
// 删除单个 skill
export async function deleteSkill(skillName, pkgName) {
    try {
        if (pkgName) {
