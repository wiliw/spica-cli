...
export function saveProjectState(workspacePath, state) {
    try {
...
export function updateProjectTodos(workspacePath, todos) {
...
export function addDecision(workspacePath, decision, reason) {
    const state = loadProjectState(workspacePath) || {
        phase: 'unknown',
        todos: [],
        decisions: [],
...
export function setProjectPhase(workspacePath, phase) {
    const state = loadProjectState(workspacePath) || {
        phase: 'unknown',
        todos: [],
        decisions: [],
...
export function loadProjectContext(workspacePath) {
    try {
...
export function saveProjectContext(workspacePath, messages) {
    try {
        ensureProjectDir(workspacePath);
        const contextPath = path.join(workspacePath, CONTEXT_FILE);
