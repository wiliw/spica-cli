...
function parseMarkdownConfig(filepath, content) {
    const config = { rawContent: content };
    // 尝试匹配各种section标题
    const sections = extractAllSections(content);
    // Project / Overview
    const project = sections.find(s => s.heading.match(/^(Project|Overview|About)/i));
    if (project) {
    ...
    // Dev / Setup / Commands
    const dev = sections.find(s => s.heading.match(/^(Dev|Development|Setup|Commands|Environment)/i));
    if (dev) {
        config.commands = {
    ...
    // Constraints / Rules
    const constraints = sections.find(s => s.heading.match(/^(Constraints|Rules|Guidelines|Important)/i));
    if (constraints) {
...
function extractCommand(section, type) {
    // 尝试多种格式
    const patterns = [
        // - Build: `npm run build`
...
// 自动检测项目类型
export function autoDetectProject(workspace) {
    const config = {};
    // Node.js / TypeScript
...
// 创建 AGENTS.md 文件
export async function createAgentsMd(workspace) {
