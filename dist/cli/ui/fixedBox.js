...
export function enableScrollRegion() {
...
export function disableScrollRegion() {
...
// 移动到输入框区域（底部）
export function moveToInputBox() {
    const height = getTerminalHeight();
...
// 显示用户输入内容
export function showInputContent(content) {
    const height = getTerminalHeight();
    const width = getTerminalWidth();
    // 清除输入行并重写
    process.stdout.write(`${ESC}[${height};1H${ESC}[2K`);
...
// 输出到滚动区域（AI 输出）
export function writeToScrollArea(content) {
    // 保存光标位置
...
// 输出一行到滚动区域
export function writeLineToScrollArea(content) {
...
// 处理终端 resize
export function handleResize(status) {
    initFixedInputBox(status);
}
// 监听 resize 事件
export function watchResize(getStatus) {
...
//# sourceMappingURL=fixedBox.js.map