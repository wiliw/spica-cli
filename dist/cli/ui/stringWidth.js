...
// 用空格填充到目标宽度（居中）
export function padCenter(str, targetWidth) {
    const currentWidth = getStringWidth(str);
...
// 对齐多行文本（左对齐）
export function alignLines(lines, width) {
...
// 获取字符串中每个字符的宽度数组
export function getCharWidths(str) {
    return Array.from(str).map(char => isFullWidth(char) ? 2 : 1);
}
// 在指定宽度位置分割字符串
export function splitAtWidth(str, width) {
    let left = '';
    let leftWidth = 0;
    for (const char of str) {
        const charWidth = isFullWidth(char) ? 2 : 1;
...
// 测试函数
export function testStringWidth() {
    const testCases = [
        ['hello', 5],
        ['你好', 4],
        ['hello你好', 9],
    ...
    testCases.forEach(([str, expected]) => {
        const actual = getStringWidth(str);
