...
export const format = {
    ...
    success: (text) => LAIN_COLORS.success(text),
    error: (text) => LAIN_COLORS.error(text),
    warning: (text) => LAIN_COLORS.warning(text),
    toolCall: (name) => LAIN_COLORS.tool(`→ ${name}`),
    toolResult: (name, success, output) => {
        const icon = success ? LAIN_COLORS.success('✓') : LAIN_COLORS.error('✗');
        return `${icon} ${name}: ${output}`;
    },
    reasoning: (content) => LAIN_COLORS.reasoning(content),
    diffFile: (path) => LAIN_COLORS.file(`📄 ${path}`),
    diffAdd: (line) => LAIN_COLORS.diffAdd(`+ ${line}`),
    diffRemove: (line) => LAIN_COLORS.diffRemove(`- ${line}`),
    permissionBox: (reason) => {
...
`;
    ...
    muted: (text) => LAIN_COLORS.muted(text),
    dim: (text) => LAIN_COLORS.dim(text),
    // 表格格式化（支持中英文对齐）
