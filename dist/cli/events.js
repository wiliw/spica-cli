...
export function setupAgentEvents(agent, interactive = false, model) {
    // 追踪 reasoning 状态
    ...
    agent.on('connection_error', (data) => {
    ...
    agent.on('tool_result', (data) => {
        ...
        if (data.syntaxErrors && data.syntaxErrors.length > 0) {
            ...
            data.syntaxErrors.slice(0, 3).forEach((err) => {
        ...
        if (outputLines.length > 5) {
            // 长输出折叠：显示前3行和摘要
            outputLines.slice(0, 3).forEach((line) => {
    ...
    agent.on('workspace_changed', (data) => {
        screen.appendScroll(LAIN_COLORS.file(`\n[DIR] Workspace: ${data.path}\n`));
    });
    agent.on('bypass_changed', (data) => {
    ...
    agent.on('agent_interrupted', (data) => {
        // 停止所有心跳
        stopHeartbeat();
        // 重置流式状态
        state.setStreamingOutput(false);
    ...
    agent.on('agent_stopped_on_error', (data) => {
    ...
    function displayTodoProgress(todos) {
        const statusIcons = {
            'completed': '✔',
            'in_progress': '◼',
            'pending': '◻',
    ...
    agent.on('context_compressed', (data) => {
        const formatTokens = (t) => t >= 1000 ? `${Math.floor(t / 1000)}k` : `${t}`;
