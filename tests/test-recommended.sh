...
AGENTS_BACKUP="$PROJECT_DIR/AGENTS_test_backup.md"
...
echo "完整版行数: $(wc -l < "$AGENTS_FILE")"
RESULTS_FULL="$RESULTS_DIR/rec_full_$(date +%Y%m%d_%H%M%S).json"
echo "[]" > "$RESULTS_FULL"
...
for i in "${!TASKS[@]}"; do
  ...
  echo "    耗时: ${duration}s"
  echo "{\"version\": \"完整版-161行\", \"task\": \"$task\", \"duration\": $duration}" >> "$RESULTS_FULL"
done
echo ""
...
echo "推荐版行数: $(wc -l < "$AGENTS_FILE")"
RESULTS_RECOMMENDED="$RESULTS_DIR/rec_$(date +%Y%m%d_%H%M%S).json"
...
echo "=== 对比摘要 ==="
echo "完整版-161行 总耗时: $full_total 秒"
